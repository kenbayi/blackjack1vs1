...
// New initializes a new gRPC user connection without interceptors.
func New(target string) (*grpc.ClientConn, error) {
	...
	// Create the gRPC user connection with necessary options
	...
	if err != nil {
		return nil, fmt.Errorf("grpc user: %w", err)
	}
